...
class MainWindow(QMainWindow):
    ...
    def load_file(self) -> None:
        """
            Jack Bellgowan, Tim Lightner and Connor Ostrowski
            the load_file method will open the employee.data cvs file and read the data from it. It uses a loop to iterate
            over each element of the file. this will create a list of data for an employee that has a type
            name, email, the amount they earn, and either a role or department or a date. this set of data is
            used to create a new Employee of the correct sub-type and add the data and will add the employee to self.data
        """
    ...
    def save_file(self) -> None:
        """
        Jack Bellgowan, Tim Lightner and Connor Ostrowski
        The save_file methodwill open the employee.data csv file and will loop over each element in the self.data and
        uses __repr()__ to return a sting in the form, type, name, email, image path, pay, and optional of
        Role and Department.
        """
...
class SalariedForm(EmployeeForm):
    """
        Jack Bellgowan, Tim Lightner and Connor Ostrowski
        the SalariedForm inherits from the EmployeeForm. This class also overrideds the fill_in() method and
        will add a "Salary:" label and the self._pay_edit from the parent. update_employee will is overwritten
        to store the yearly salary entered into the current employee object

        Attributes:
            parent (QApplication): the parent application layout
            employee (Employee): the base employee form
    """

    def __init__(self, parent: QApplication, employee: Employee):
        """
        Constructor for the SalariedForm
...
class ExecutiveForm(SalariedForm):
    """
        Jack Bellgowan, Tim Lightner and Connor Ostrowski
        the exectuve form inhertis from the SalariedForm and adds a label with the test "Role:" and a QComboBox and this
        box holds the enumeration value for each Department type. Overrides the fill_in() to set the employee role to the
        given value read in from the file. Overrides the update_employee() to store the new selected of the current employee
...
class ManagerForm(SalariedForm):
    """
        Jack Bellgowan, Tim Lightner and Connor Ostrowski
        The ManagerForm inherits from SalariedForm. this adds a labelthe the text "Head of department:" and the
        QComboBox that holds each of the values from the Department enumerations. Overrides the the fill_in() to
        set the emloyee department to the one read in from the file. Also overrides update_employee() to store the
        the selected value into the employee object when the user selected update.
    ...
    def __init__(self, parent: QApplication, employee: Employee):
        """
        Constructor for the ManagerForm
...
class HourlyForm(EmployeeForm):
    """
    Jack Bellgowan, Tim Lightner and Connor Ostrowski
    The HourlyForm inherits from EmployeeForm. overrides the fill_in() to add a text "Hourly wage:" label and a
    self._pay_edit QLineEdit created in the parent form. Also overrides update_employee()to storethe hourly wage
    entered into the current employee object
    ...
    def __init__(self, parent: QApplication, employee: Employee):
        """
        Constructor for the HourlyForm
...
class TempForm(HourlyForm):
    """
    Jack Bellgowan, Tim Lightner and Connor Ostrowski
    TempForm inherits from HourlyForm and overrides the fill_in() to add a label with the text "Last Day:" and will
    add a label with the emoloyees last date as a read form the file. Since this is read only there is no need for
    a QlineEdit
...
class PermanentForm(HourlyForm):
    """
    Jack Bellgowan, Tim Lightner and Connor Ostrowski
    PermanentForm inherits form HourlyForm. will override the fill_in() to add a label with the text "Hired Date:"
    and a label with the date of the employee's hired date as a a read fro the file.
    ...
    def __init__(self, parent: QApplication, employee: Employee):
        """
        Constructor for the PermanentFrom
